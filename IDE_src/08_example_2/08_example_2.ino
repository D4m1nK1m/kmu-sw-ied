...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < 200 && dist_raw > 100) {
    analogWrite(PIN_LED, 255.0-255.0*(dist_raw-100.0)/100.0);
    Serial.println(255.0*(dist_raw-100.0)/100.0);
  }
  else if(dist_raw < 300 && dist_raw > 200){
    analogWrite(PIN_LED, 255.0-255.0*(300.0-dist_raw)/100.0);
  }else{
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading == 0.0|| reading > dist_max)
    reading = last_reading;
  else {
    reading += (last_reading - reading)/2;
    if(reading < dist_min) reading = 0.0; // return 0 when out of range.
